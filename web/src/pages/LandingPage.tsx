        ...
        <div className='mx-auto w-full max-w-xl space-y-5'>
          ...
          {!loading && isAuthenticated ?
            <div className='space-y-3 rounded-xl border border-black/10 bg-white/60 p-4'>
              ...
              />
              <p className='text-xs text-muted'>
                Etape suivante: connecter Strava. Tu peux aussi definir des
                credentials OAuth Strava personnalises.
              </p>
              {!hasCustomStravaCredentials ?
                <p className='text-xs text-amber-700'>
                  Credentials Strava non configures: complete la page Strava
                  Credentials avant la connexion OAuth.
                </p>
              : null}
          ...
          {!loading && !isAuthenticated ?
            <>
              <div className='grid grid-cols-2 gap-2 rounded-xl border border-black/10 bg-white/60 p-2'>
                ...
                <button
                  className={`h-10 rounded-lg text-sm transition ${
                    mode === 'register' ? 'bg-ink text-white' : (
                      'hover:bg-black/5'
                    )
                  }`}
                  onClick={() => {
